...
2. Confirm the extension menu can open the `actions.json` menu.
3. If you are using the hosted agent, confirm the Settings tab says the OpenAI
   key is saved and the Agent tab can start a voice session.
...
Cause: the Agent tab and Settings tab are reading different saved state, or the
key was not saved before the Agent tab refreshed.

Fix:

1. Open the `actions.json` menu.
2. Open **Settings**.
3. Paste the OpenAI API key.
4. Save it.
5. Confirm the status says the key is saved.
6. Return to **Agent** and start a new session.

If it still fails, close the menu, reopen it from the extension popup, and check
`runtime.session.log` for key-state events.
...
- For the hosted agent, reopen the menu and check Settings. The hosted agent
  should load tools from the extension runtime and uploaded storage even when no
...
1. Upload the whole `actions.json.storage` checkout, not a single site folder.
2. Confirm the checkout contains the relevant scope repository and site folder.
3. Open the target website.
4. Reopen the `actions.json` menu or press the storage refresh control.
...
The extension should recreate the `actions.json` menu after navigation because
...
2. Use the popup voice controls to check whether the offscreen session is still
   live.
3. Reopen the `actions.json` menu.
